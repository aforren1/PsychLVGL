...
| ImageFromTexture | `img = PsychLVGL('ImageFromTexture', glTex, w, h)` | Image handle that draws an existing `GL_TEXTURE_2D` texture, stored bottom row first, with no copy. The texture stays the caller's. `m/PsychLVGLImageFromTexture.m` makes one from a Psychtoolbox texture. |
...
| `m/PsychLVGLImageFromTexture.m` | `img = PsychLVGLImageFromTexture(win, tex)`. Phase 2. Reads the OpenGL name, target and orientation of a Psychtoolbox texture with `Screen('GetOpenGLTexture')` and calls `ImageFromTexture`. Refuses rectangle and transposed textures with `psychlvgl:Texture`. |
...
| `psychlvgl:Texture` | `PsychLVGLImageFromTexture` got a texture LVGL cannot draw: not `GL_TEXTURE_2D`, or stored transposed. Phase 2. |
...
| D41 | Images from Psychtoolbox textures need a second vendored patch, `patches/lvgl/0002-nanovg-image-from-gl-texture.patch` (64 lines in 4 files). It adds the image flag `LV_IMAGE_FLAGS_GL_TEXTURE`: an `lv_image_dsc_t` with that flag carries a `uint32_t` OpenGL texture name instead of pixels, and the NanoVG image cache wraps the name with `nvglCreateImageFromHandle` and `NVG_IMAGE_NODELETE | NVG_IMAGE_FLIPY` on every draw, releasing only the NanoVG record at the end of the frame. The descriptor is therefore an ordinary image source for `lv_image_set_src`, with rotation, scale, recolor and clip radius. Without a patch LVGL 9.6 offers no image source backed by a texture: every image goes through an image decoder that yields CPU pixels, which the NanoVG unit uploads with `nvgCreateImage`. The only upstream way to draw an existing texture is the `3dtexture` widget with `LV_USE_3DTEXTURE`. It was rejected: it is a widget, not an image source; `lv_draw_nanovg_3d` ends the NanoVG frame for every texture; and it draws through `lv_opengles_render` with `rb_swap` fixed to true, which swaps red and blue of a Psychtoolbox RGBA texture, measured by reading the source. `lv_opengles_texture_create_from_texture_id`, which section 13 names, creates a display, not an image. The texture must be `GL_TEXTURE_2D` and stored bottom row first, which is what Psychtoolbox gives with `Screen('MakeTexture', win, M, [], 1, [], 1)`; its default is a rectangle texture, and `specialFlags` 1 alone stores the image transposed (measured with `Screen('GetOpenGLTexture')` on Psychtoolbox 3.0.22). `m/PsychLVGLImageFromTexture.m` checks both and raises the new `psychlvgl:Texture`. Images are at most 16383 pixels on a side, because the image header stores the row stride in 16 bits. The software variant has no OpenGL, so there `ImageFromTexture` returns a transparent image of the given size with the same handle rules. The generator reads the patched headers, so `src/psychlvgl_enums.c` lists `LV_IMAGE_FLAGS_GL_TEXTURE`. `ImageFromArray` is added as well: a uint8 HxW, HxWx3 or HxWx4 array becomes an ARGB8888 copy the MEX owns. With `LV_CACHE_DEF_SIZE 0` the NanoVG unit uploads such an image again every time it draws it, so a large or often redrawn image belongs in a texture. | The phase 2 brief allows a small second patch when a texture-backed image source needs one. It keeps the lv_image features and the colour order of the texture, and needs no pixel to cross the CPU. The upright rule follows from `NVG_IMAGE_FLIPY` and matches Psychtoolbox's own normal orientation, so the same texture looks the same in `Screen('DrawTexture')` and in the panel. |
...
| D47 | Tests for phase 2. The no-GL suite gains `test_chart`, `test_styles`, `test_fonts` and `test_images`, and `test_helpers` checks `PsychLVGLImageFromTexture` against a stub `GetOpenGLTexture`; 507 checks pass under MATLAB R2023a and Octave 10.1 on Windows, up from 355. The GL suite gains `test_gl_chart`, `test_gl_style`, `test_gl_image` and `test_gl_font`; 59 checks pass with Psychtoolbox 3.0.22 on an OpenGL 4.6 context. `tests/native/smoke_gl.c` gains a second scene with a shared style, a line chart, a texture image and a TTF label, and it reads which framebuffer row holds panel row 0 before it checks the image orientation. The TTF tests use `examples/libs/tiny_ttf/Ubuntu-Medium.ttf` from the LVGL submodule, so they need no font of the host. | The brief's test list. The smoke test is what CI's `smoke-gl-linux` and `smoke-gl-macos` run, so the new draw paths get automated GL coverage on Mesa and on the Apple software renderer. |
