...
- Upstream `lv_opengles_init()` is not GL 2.1 clean, whatever `LV_NANOVG_BACKEND`
  ...
  `glGenVertexArrays` falls back to `glGenVertexArraysAPPLE` and the framebuffer
  functions to their `EXT` forms when the core names are absent, and Apple's
  legacy profile exports both extensions.
