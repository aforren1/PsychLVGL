<#
.SYNOPSIS
Release PsychLVGL 0.2.0: set the version, verify, push, tag, and check the published zip.
...
$Name        = 'PsychLVGL'                           # tag and release title prefix
$MexName     = 'PsychLVGL'                           # the MEX function
$VersionField = 'psychlvgl'                          # field of ('Version') that carries the version
$SetupName   = 'PsychLVGLSetup'                      # setup function at the zip root
$WindowsZip  = 'psychlvgl-matlab-windows.zip'        # asset checked after the release
$VersionEdits = @(
    @{ File = 'src/psychlvgl.c'; Pattern = '#define PSYCHLVGL_VERSION "\d+\.\d+\.\d+"'; Replacement = '#define PSYCHLVGL_VERSION "{V}"' }
)
$NeedsGen    = $false                                # the generator stamps the version into sources
$LocalTestRuns = @(
    @{ Label = 'MATLAB build test and compile-only'; Exe = 'matlab'; Args = @('-batch', 'build test; build compile-only') },
    @{ Label = 'Octave build test and compile-only'; Exe = 'octave'; Args = @('--eval', 'build test; build compile-only') }
)
